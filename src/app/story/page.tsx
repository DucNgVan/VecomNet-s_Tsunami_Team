  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="pt-28 pb-20 px-4 sm:px-8 max-w-7xl mx-auto space-y-16 relative z-10">
        ...
        {/* Ocean Wave Ribbon */}
        <OceanWaveDivider height={40} colorVariant="ocean" className="opacity-80" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <OceanFloatingCard delay={0} duration={6} distance={5}>
          <div className="h-full p-6 rounded-3xl bg-white/95 backdrop-blur-sm border border-slate-200/90 shadow-[0_4px_16px_rgba(11,30,59,0.04)] hover:shadow-card hover:border-sky-300 transition-all text-center group">
            <div className="text-3xl sm:text-4xl font-black font-mono text-sky-700 group-hover:scale-105 transition-transform">
        ...
        <OceanFloatingCard delay={0.2} duration={5.5} distance={4}>
          <div className="h-full p-6 rounded-3xl bg-white/95 backdrop-blur-sm border border-slate-200/90 shadow-[0_4px_16px_rgba(11,30,59,0.04)] hover:shadow-card hover:border-teal-300 transition-all text-center group">
            <div className="text-3xl sm:text-4xl font-black font-mono text-teal-700 group-hover:scale-105 transition-transform">
        ...
        <OceanFloatingCard delay={0.4} duration={6.5} distance={5}>
          <div className="h-full p-6 rounded-3xl bg-white/95 backdrop-blur-sm border border-slate-200/90 shadow-[0_4px_16px_rgba(11,30,59,0.04)] hover:shadow-card hover:border-emerald-300 transition-all text-center group">
            <div className="text-3xl sm:text-4xl font-black font-mono text-emerald-700 group-hover:scale-105 transition-transform">
        ...
        <OceanFloatingCard delay={0.6} duration={5.8} distance={4}>
          <div className="h-full p-6 rounded-3xl bg-white/95 backdrop-blur-sm border border-slate-200/90 shadow-[0_4px_16px_rgba(11,30,59,0.04)] hover:shadow-card hover:border-amber-300 transition-all text-center group">
            <div className="text-3xl sm:text-4xl font-black font-mono text-amber-700 group-hover:scale-105 transition-transform">
      ...
      <div className="space-y-10 relative">
        {/* Central Connecting Water Current Line */}
        <div className="hidden lg:block absolute left-[54px] top-32 bottom-20 w-0.5 bg-gradient-to-b from-sky-400 via-teal-500 to-emerald-400 opacity-30 z-0">
          <motion.div
            animate={{ y: ["0%", "100%"] }}
            transition={{ duration: 6, repeat: Infinity, ease: "linear" }}
            className="w-2 h-8 -left-[3px] absolute bg-cyan-300 rounded-full blur-[2px]"
          />
        </div>
        ...
        <div className="text-center max-w-2xl mx-auto space-y-2">
          ...
          <p className="text-xs sm:text-sm text-slate-500">
            Hành trình thăng hoa từ đáy biển sâu 30 mét đến sàn diễn thời trang bền vững
          </p>
        </div>

        <div className="space-y-8 relative z-10">
          {RECYCLING_JOURNEY.map((step, idx) => {
            const depthInfo = stepDepths[idx];
            return (
              <motion.div
                key={step.stepNumber}
              ...
              >
                {/* Oceanic Depth Watermark Badge in top-right */}
                <div className="absolute top-4 right-4 sm:top-6 sm:right-8 flex items-center gap-2">
                  <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-50 text-sky-800 text-xs font-mono font-bold border border-sky-200 shadow-xs">
                    <Waves className="w-3.5 h-3.5 text-sky-600 animate-pulse" />
                    <span>{depthInfo.depth}</span>
                  </span>
                  <span className="hidden sm:inline-block px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 text-[11px] font-mono font-medium">
                    {depthInfo.badge}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                  ...
                  {/* Right Column: Active Sonar for Step 1 or Interactive Visual */}
                  <div className="lg:col-span-4 flex flex-col items-center justify-center p-6 rounded-2xl bg-gradient-to-b from-slate-50 to-sky-50/40 border border-slate-200/80 text-center relative overflow-hidden">
                    {idx === 0 ? (
                      <div className="py-2 flex flex-col items-center">
                        <OceanSonarRadar size={110} label="ĐÁY BIỂN -30M" />
                        <div className="text-xs text-slate-500 font-medium mt-8">Định vị sóng âm sonar</div>
                        <div className="text-xs text-sky-700 font-bold font-mono">
                          Trục vớt không chạm san hô
                        </div>
                      </div>
                    ) : (
                      <>
      ...
      <div className="p-8 sm:p-12 rounded-3xl bg-white/95 backdrop-blur-md border border-slate-200/90 shadow-[0_12px_40px_rgba(11,30,59,0.06)] relative overflow-hidden">
        {/* Ambient Wave Inset */}
        <OceanWaveDivider height={28} colorVariant="teal" className="absolute top-0 inset-x-0 opacity-40" />
